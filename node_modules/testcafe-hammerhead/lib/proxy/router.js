...
class Router {
    ...
    _registerRoute(route, method, handler) {
        ...
        if (isRouteWithParams && typeof handler === 'function')
            ...
            if (typeof handler !== 'function') {
                ...
                handler.etag = (0, crypto_md5_1.default)(handler.content);
    ...
    _unregisterRouteWithParams(tokens, method) {
        ...
        const routeIndex = this.routesWithParams.findIndex(routeWithParam => {
            return (0, lodash_1.isEqual)(routeWithParam.re, re) && (0, lodash_1.isEqual)(routeWithParam.paramNames, paramNames);
        });
        if (routeIndex !== -1)
            this.routesWithParams.splice(routeIndex, 1);
    }
    _route(req, res, serverInfo) {
        const routerQuery = `${req.method} ${(0, url_1.getPathname)(req.url || '')}`;
        const route = this.routes.get(routerQuery);
        if (route) {
            if (route.isStatic)
                (0, http_1.respondStatic)(req, res, route.handler, this.options.staticContentCaching);
            else
